...
TEST(tests, vector_at_out_of_bounds)
{
	...
	EXPECT_THROW(vector.at(76), std::invalid_argument) << "vector at doesn't raises an exception accessing out of bounds";
}

TEST(tests, vector_at_init_out_of_bounds)
{
	dsa::Vector<int> vector;
	EXPECT_THROW(vector.at(1), std::invalid_argument) << "vector at doesn't raises an exception accessing out of bounds";
}
